...
def run(targets, output, count=3, moves=5000, seconds=30):
    if Path(output).exists():raise FileExistsError(output)
    entries=json.loads(Path(targets).read_text())['candidates']
    selected=[];seen=set()
    for r in entries:
        if r['hfk_check']['fibered']:continue
        key=r['numerical_isometry_signature'] or r['diagram_signature']
    ...
    result={'status':'BOUNDED_ENDPOINT_NOMINATION_ONLY','input_targets':targets,
            'length':6,'twists':2,'path':'shortest','simplification':'basic',
            'max_moves_per_target':moves,'seconds_per_target':seconds,'runs':[]}
    for r in selected:
        J=snappy.Link(r['endpoint_pd']);normalize_crossing_labels(J)
        n=deaths=hfk_passes=0;hits=[];t0=time.monotonic();stop='enumeration_finished';cache=set()
        for L,band in banded_links(J,2,6,'shortest'):
        ...
        row={'source_index':r['index'],'crossings':r['crossings'],'bands_tested':n,
             ...
             'K1_HFK_matches':hfk_passes,'numerical_hits':hits,'stop':stop,

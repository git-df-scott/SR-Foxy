...
class Jet:
    ...
    def __init__(self, coefficients=0):
        ...
        for k in range(len(d)-1, N-1, -1):
            c = d[k] if self.modulus is None else d[k] % self.modulus
            for j in range(self.order+1):
                d[k-N+2*j] -= c*comb(self.order, j)
        self.d = tuple((d[i] if i < len(d) else 0) for i in range(N))
        if self.modulus is not None:self.d=tuple(c%self.modulus for c in self.d)
...
def probe(link, expected_product, exact=False):
    ...
    Jet.order=m+1
    Jet.modulus=None if exact else 32
    ...
    try:
        ...
        starts=starts[::max(1,len(starts)//200)][:200]
    ...
    for k in range(len(d)-1,2*m-1,-1):
        c=d[k] if exact else d[k]%32;quot[k-2*m]=c
        for j in range(m+1):d[k-2*m+2*j]-=c*comb(m,j)
    remainder=d[:2*m] if exact else [x%32 for x in d[:2*m]]
    # delta=q^-1(q^2+1); convert h-quotient to delta-quotient.
    product=Jet(quot)*Jet([0,1])**m
    a=sum(c*(-1)**(i//2) for i,c in enumerate(product.d) if i%2==0)
    b=sum(c*(-1)**(i//2) for i,c in enumerate(product.d) if i%2)
    return {'components':m,'crossings':len(link.crossings),'jet_mod_32':[c%32 for c in value.d],
            'exact_integer_arithmetic':exact,'exact_divisibility_remainder':remainder if exact else None,
            'exact_quotient_at_i':[a,b] if exact and not any(remainder) else None,
            'morse_width':encoded.width,'max_retained_states':max_states,'exhaustion_starts_tested':len(choices),
            'layout_seed':20260917,
            'divisibility_remainder_mod_32':[c%32 for c in remainder],
            'quotient_at_i_mod_32':[a%32,b%32], 'expected_mod_32':expected_product%32,
            'ribbon_obstructed_mod_32':bool(any(c%32 for c in remainder) or b%32 or a%32!=expected_product%32),
            'ribbon_divisibility_obstructed_exact':bool(any(remainder)) if exact else None,
...
if __name__=='__main__':
    ...
    print(json.dumps(probe(link,data['expected_product'],data.get('exact',False))),flush=True)

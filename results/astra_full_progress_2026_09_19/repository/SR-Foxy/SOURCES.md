...
- **Theorem 5.5, exact hypotheses** (verbatim from the [S06] Appendix A restatement, LaTeX source line 517, re-fetched 2026-09-16). *For `i = 1, ..., n`, let `K_i` be a prime fibered knot in a homotopy 3-sphere `M_i` **satisfying one of the following**: (a) `K_i` is minimal with respect to `≥` among all fibered knots in homology spheres; (b) there is no `f(t) ∈ Z[t] \ {±t^k}_k` such that `f(t)f(t^{-1}) | Δ_{K_i}(t)`. If `K_1 # ⋯ # K_n` is homotopically ribbon, then `{1, ..., n}` can be paired into `⊔_{s=1}^{m} {i_s, j_s}` with `K_{i_s} = conj(K_{j_s})`, where `conj(K_j) = (M_j^r, K_j^r)`.* Here `K_1 ≥ K_0` is **homotopic ribbon concordance**: a 4-manifold `X` with `H_*(X) ≅ H_*(S^3 × I)` and an embedded annulus `A` with `(∂X, A ∩ ∂X) = (M_1, K_1) ⊔ (M_0^r, K_0^r)`, `π_1(M_1 ∖ K_1) → π_1(X ∖ A)` surjective and `π_1(M_0 ∖ K_0) → π_1(X ∖ A)` injective. `K` is *homotopically ribbon* if `K ≥ U`.
- **The quantifier is over every `i`.** This is load-bearing and was dropped in five planning documents; see `ERRATA_2026-09-16.md` (E16-1). Consequences, both directions: a **nontrivial ribbon `J` satisfies neither (a) nor (b)** — not (a) because `J ≥ U` with `U ≠ J` fibered in `S^3`, not (b) because Fox–Milnor makes `Δ_J` itself a nonunit norm — so Theorem 5.5 **cannot obstruct ribbonness of `K # J` for any nontrivial ribbon `J`**, which is exactly what the Teichner lane requires. Conversely it **does** apply when every prime summand of `J` satisfies an alternative, e.g. `J = L # (-L)`.
- By the Remark following the theorem in [S06], geometrization makes each `M_i = S^3`. Per [S08]'s reading of the remark after [S07, Lemma 1.3] and [Baker 2016, Thm 3], "among all fibered knots in homology spheres" in (a) may be relaxed to "among all fibered knots in `S^3`".
- Corollary (of the above, [S06, Cor. 4.3 / cor:Miyazaki]): fibered `K_0`, `K_1` in `S^3` with **irreducible** Alexander polynomials and `K_0 # conj(K_1)` ribbon forces `K_0 = K_1`. Irreducibility is what supplies alternative (b) for the `A_n(6_3)` family, so the non-ribbon certificate on `D_{n,m}` is unaffected by E16-1.
- Evidence: `SECONDARY RESTATEMENT ×2, VERIFIED` — [S06] Appendix A LaTeX source and, independently, [S08] arXiv:2507.20455 LaTeX source (proof of their Prop., which establishes minimality for *every* summand before invoking 5.5). **Downgraded 2026-09-16 from `PEER-REVIEWED PRIMARY`:** the AMS full text was *not* retrieved by this repository — `ams.org` returns a Cloudflare 403, as already recorded for [S29] — and the earlier "checked through the original" cannot be substantiated from anything in this clone. Alternative (b) is machine-checked over `Z` for the relevant knots by `scripts/miyazaki_hypothesis_check.py`.

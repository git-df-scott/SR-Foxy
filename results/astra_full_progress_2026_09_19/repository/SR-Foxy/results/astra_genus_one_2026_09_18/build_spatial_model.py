...
base=json.loads((P/'planar_base.json').read_text());pos={tuple(k):np.array([v[0],-v[1]],float) for k,v in base['positions']} # reverse NetworkX's planar orientation to match the archived signed PD
paths=[];seglabels=[]
...
for h,tt in enumerate(targets):
 ...
 corner=end[:2]+enddir*10
 remote=corner+np.array([-enddir[1],enddir[0]])*10
 reserved=LineString([remote,corner,end[:2]]).buffer(EPS*100,cap_style=2,join_style=2).difference(Point(remote).buffer(EPS*200,resolution=4))
 obstacles=[old,reserved]+[Point(d['center'][:2]).buffer(d['radius']*1.5,resolution=4) for d in alltargets]
 pts.extend(visibility_route(pts[-1],remote,obstacles)[1:]);pts.extend([corner,end[:2]])

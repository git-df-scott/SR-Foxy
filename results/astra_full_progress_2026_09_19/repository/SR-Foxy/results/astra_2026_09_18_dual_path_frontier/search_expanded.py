...
import itertools,contextlib,io,time
...
summary=[]
for pi,path in enumerate(paths):
 rr=[x for x in rows if x["path_index"]==pi]
 if not rr:continue
 summary.append({"path_index":pi,"path":path,"tested":len(rr),"expected":2**(len(path)-2),"trace_rejected":sum(not x["trace_match"] for x in rr),"trace_pass":[{"bits":x["band"]["arc_is_under"],"surgery":x["surgery"]} for x in rr if x["trace_match"]]})
print(json.dumps({"scope":"fixed first band and second endpoints; simple dual paths with <=6 internal edges; q0 trace and r=1 surgery gates only","elapsed":time.monotonic()-started,"path_count":len(paths),"expected":sum(2**(len(p)-2) for p in paths),"tested":len(rows),"complete":len(rows)==sum(2**(len(p)-2) for p in paths),"summary":summary,"CE":False}),flush=True)

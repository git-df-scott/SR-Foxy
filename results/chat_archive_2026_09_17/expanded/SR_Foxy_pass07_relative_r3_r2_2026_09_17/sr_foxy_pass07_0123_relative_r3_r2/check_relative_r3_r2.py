...
def faces(A,active):
    ...
    while corners:
        ...
        while True:
            ...
            cur=A[c][(p+1)%4]  # Spherogram CrossingStrand.next_corner()
            if cur==st: break
        out.append(face)
    return out
...
def bounded_any_eta_r2_search(A,active,max_depth=4):
    """Exhaust RIII-only states and ask if ANY eta-eta RII becomes available."""
    q=deque([(A,set(active),[])]); seen={canonical(A,active)}; expanded=0
    while q:
...
def main():
    ...
    # At both mixed crossings used below, R occupies under-ports 0,2 and eta an over-port 1,3.
    for c in (17,18):
        assert cmap[(c,0)]==rcomp and cmap[(c,2)]==rcomp
        assert cmap[(c,1)]!=rcomp and cmap[(c,3)]!=rcomp
    # pass06's alleged obstruction lives in the triangle 17,18,26.
    triangles=[t for t in possible_r3(A,active) if {c for c,p in t}=={17,18,26}]
    assert triangles==[[(17,3),(18,2),(26,3)]],triangles
    tri=triangles[0]
    # Both mixed crossings have eta at odd ports: eta is over R at each.
    assert tri[0]==(17,3) and tri[1]==(18,2)
    assert A[17][0]==(18,2)  # the R strand directly joins the two mixed crossings
    A1=r3(A,tri)
    ...
    # The eta-only four-crossing residual still reduces to the unlink if R is deleted.
    etaA=adjacency(eta1); etaactive=set(etaA)
    eta_r2=[]
    while etaactive:
    ...
    assert r2_candidates(A2,active2)==[]  # no immediate full-link RII after the certified move
    ...
    result={
      "status":"FIRST_RELATIVE_UNLINK_MOVE_CERTIFIED_NO_COUNTEREXAMPLE",
      "counterexample":False,
      "input_main_commit":"9d4634224db3c32dce419e67510c994111bd7502",
      "bands":["a40a423e_0_0","d0826c36_0_0"],
      "initial_full_crossings":62,
      "initial_eta_sublink_crossings":6,
      "certified_full_link_isotopy":[
        {"move":"RIII","triangle":tri,"crossing_set":[17,18,26]},
        {"move":"RII","data":hits[0],"crossing_set":[26,56]}
      ],
      ...
      "after_move_full_crossings":60,
      "after_move_eta_sublink_crossings":4,
      "after_move_eta_sublink_pd":eta1,
      "eta_only_remaining_RII_moves_after_deleting_R":eta_r2,
      "full_link_immediate_RII_after_move":[],
      "rIII_only_unlock_search_for_next_eta_pair":search,
      "rIII_only_search_for_any_eta_RII":any_search,
      "scope":[
        "This corrects pass06's claim that the first eta RII cancellation is blocked by R.",
        "The first RIII is supported off R because R is the bottom strand at both mixed crossings; together with the eta-eta RII this certifies the first eta cancellation by an isotopy in S3\\R. It still does not supply an annulus in the product-disk exterior.",
        "The remaining four-crossing eta configuration is not certified unlink relative to R.",
        "The depth-4 RIII search is finite diagram search only, not an obstruction theorem.",
        "No surgery framing, boundary-knot identity, Park 0-standardness, or slice disk is certified."
      ],

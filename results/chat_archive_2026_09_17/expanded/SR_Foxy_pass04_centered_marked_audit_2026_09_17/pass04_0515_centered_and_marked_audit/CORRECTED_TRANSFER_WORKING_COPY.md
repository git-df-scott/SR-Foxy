> AMENDED WORKING COPY, 17 September 2026 UTC. The original is archived unchanged.
> Section 3 equation (2) now uses (r+1)^n rather than r^n; the top-degree
> reciprocal-series descriptions use the same centered coordinate. See
> MOMENT_CORRECTION.md for the proof and counterexample to the old identity.
> Earlier computation counts in section 9 are historical, not reruns in this
> pass. This is a research derivation, not externally peer-reviewed work.
...
annihilates every reflection-parity polynomial of degree <n and parity epsilon. Indeed it annihilates P_s for s<n by comparing the coefficient of z^s. On this reflection-parity subspace, the centered top-degree value is
...
For completeness, the coefficient extraction from section 3 is as follows. In the even case the top-degree part of the reciprocal series is the reciprocal of cos((r+1)t/2). In the odd case, after the t P_1 factor is removed, it is the reciprocal of sin((r+1)t/2)/((r+1)t/2). Using (2), all terms of degree below n disappear. The coefficient multiplying e_j(K)e_k(J) in e_n(K#J) is

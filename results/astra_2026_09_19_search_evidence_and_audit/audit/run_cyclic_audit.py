...
for g in groups:
 ...
 if sets[0]&sets[1]:raise ValueError('Unseparated endpoints remain')
 result['buckets'].append({'bucket':g['bucket'],'counts':[len(g[s]) for s in ['left_nodes','right_nodes']],'profiles_by_side':[[list(k) for k in sorted(s)] for s in sets],'shared_profiles':False})

...
rules={}
for ri,rel in enumerate(C['source_relators']):
...
word=tuple(W['correction_source_word'])
steps=[]
...
assert word
print(json.dumps({'scope':'Only a deterministic rewrite, not shortest-word or conjugacy search','initial_word':W['correction_source_word'],'final_word':word,'initial_length':len(W['correction_source_word']),'final_length':len(word),'steps':steps},indent=2))

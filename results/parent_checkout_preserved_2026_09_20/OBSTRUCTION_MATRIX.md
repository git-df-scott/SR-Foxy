...
| Turaev multiplace Seifert form | **APPLIES BUT VANISHES** logically: Turaev Lemma 7.2 applies when the slice-disk commutator map is epic, which follows from the known handle-ribbon disk's π₁ epimorphism [S01,S02,S20] | **APPLIES BUT VANISHES** whenever the asserted handle-ribbon π₁ epimorphism is verified | **NOT YET COMPUTED** | **NOT YET COMPUTED** |
...
| Ribbon number / fusion number | **COMPUTED / INCONCLUSIVE**: only a bounded diagrammatic search (simple paths mostly length ≤6; at most four bands), with no complexity bound or completeness [S03,§§2.2–2.8] | **COMPUTED / INCONCLUSIVE**: algorithms fail even on known ribbon GST knots [S16] | **UNKNOWN** | **UNKNOWN** |

# Corrected draft: all-degree transfer for two Eisermann tests

17 September 2026 UTC. **NO COUNTEREXAMPLE.**

**Status: corrected draft after a second computational and mathematical audit; not externally reviewed.** This is an original derivation in this research pass, not a cited published theorem and not a claim of publication novelty. The finite calculations and direct controls below test the argument; they do not replace its all-degree proof. A second mathematical reader should attack the proof before the campaign treats the entire test family as closed. No previous result or research file is overwritten.

> Correction in this pass: the original section 3 equation (2) used `r^n` instead of `(r+1)^n` and was false. The centered identity and leading-law extraction are repaired below. See `CENTERING_ERRATUM.md` for explicit counterexamples and the independent checks. Historical computation counts in section 9 describe the original pass, not new runs. No original file is overwritten.

Original input repository commit: `0eb81a1a15147b2059b9028d0bbc0f4173f0ad33`.
...
annihilates every reflection-parity polynomial of degree <n and parity epsilon. Indeed it annihilates P_s for s<n by comparing the coefficient of z^s. Writing u=r+1, its top-degree value on the centered monomial is

    L_n((r+1)^n)=(-1)^((n-epsilon)/2) 2^n n!.                       (2)
...
For completeness, use u=r+1 throughout the coefficient extraction from section 3. The polynomials involved have ordinary parity in u, so their lower-degree terms belong to the annihilated centered polynomial space. In the even case the top-degree part of the reciprocal series is the reciprocal of cos(ut/2). In the odd case, after the t P_1 factor is removed, it is the reciprocal of sin(ut/2)/(ut/2). Using (2), all terms of degree below n disappear. The coefficient multiplying e_j(K)e_k(J) in e_n(K#J) is

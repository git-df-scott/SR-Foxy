#!/usr/bin/env python3
"""Truncated unreduced Jones in Z[x]/(x^6), v=1+x.
...
class Jet:
    ...
    def __init__(self, coefficients=0):
        ...
        self.d=tuple((coefficients[i] if i<len(coefficients) else 0) for i in range(self.order))
...
def probe(link):
    ...
    return {'coefficients_integer':list(value.d),'crossings':len(link.crossings),'morse_width':encoded.width,'max_retained_states':max_states,'chosen_start_index':chosen_index,'layout_score':cost(chosen),'starts_tested':len(choices),'scope':'Exact integer unreduced Jones at v=1+x modulo x^6.'}

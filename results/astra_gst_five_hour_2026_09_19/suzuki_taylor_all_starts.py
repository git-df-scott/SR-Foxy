...
def probe(link):
    ...
    return {'coefficients_mod101':list(value.d),'crossings':len(link.crossings),'morse_width':encoded.width,'max_retained_states':max_states,'chosen_start_index':chosen_index,'layout_score':cost(chosen),'starts_tested':len(choices),'scope':'Unreduced Jones at v=1+x modulo x^6 and101; a zero residue is inconclusive.'}

...
def probe(link):
    ...
    return {'coefficients_integer':list(value.d),'crossings':len(link.crossings),'morse_width':encoded.width,'max_retained_states':max_states,'chosen_layout_index':chosen_index,'morse_events':chosen.events,'layout_seeds':[20260920,20260921,20260922,20260923],'layout_score':cost(chosen),'starts_tested':len(choices),'scope':'Exact integer unreduced Jones at v=1+x modulo x^6.'}

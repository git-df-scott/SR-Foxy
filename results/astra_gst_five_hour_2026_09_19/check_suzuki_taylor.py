...
from suzuki_taylor_jet import probe,Jet
v=S.Symbol('v')
def convert(expr):
 out=Jet(0)
 ...
 return out
name=sys.argv[1];ci=int(sys.argv[2]);d=p/'suzuki_mixed'/f'{name}_c{ci}';inputs=json.loads((d/'INPUT.json').read_text());polys=json.loads((d/'POLYNOMIALS.json').read_text());L=snappy.Link(inputs['full_cable']['pd']);L.simplify('global');(d/'TAYLOR_INPUT_PD.json').write_text(json.dumps(L.PD_code())+'\n');start=time.monotonic();r=probe(L)
...
(d/'TAYLOR_RESULT.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r),flush=True)
